### **【改訂版 v5.0】第1章: Reactの核心を掴む 〜コンポーネント、JSX、そして宣言的UIの力〜**

#### **この章で到達するレベル**

この章を読み終えたあなたは、単にReactの書き方を知っているだけでなく、「**なぜReactがこのように設計されているのか**」を自分の言葉で説明できるようになります。あなたは、以下の問いに自信を持って答えられるようになるでしょう。

*   jQuery時代のアプローチとReactの「宣言的UI」は、本質的に何が違うのか？その背景にある仮想DOMと再調整の仕組みとは？
*   JSXはブラウザでどのように解釈されるのか？その変換プロセスと、リスト表示で`key`属性がなぜ重要なのか？
*   コンポーネントを設計するときの「ちょうど良いサイズ」とは？単一責任の原則をReactコンポーネントにどう適用すればよいか？
*   `props`が「読み取り専用」であるべきなのはなぜか？そのルールが、アプリの予測しやすさやメンテナンス性にどう貢献するのか？
*   `useState`の更新が非同期なのはなぜか？それが引き起こす問題と、関数更新（Updater Function）が必要になる場面とは？
*   複数のコンポーネントで状態を共有するための基本戦略「リフティング・ステート・アップ」とは何か？
*   Reactのレンダリングにおける「レンダーフェーズ」と「コミットフェーズ」の役割分担とは？

この章は、あなたがチームの他のメンバーにReactの基本を教えたり、技術選定の議論に参加したりするための、強固な理論的土台を築くことを目的としています。

---

### **【第1部：Reactの思想と基本コンポーネント】**

この部では、Reactの根幹にある「考え方」と、それを実現するための基本的な部品である「コンポーネント」「JSX」について学びます。表面的な使い方だけでなく、内部の仕組みや設計思想にまで踏み込んでいきましょう。

---

### **1.1 宣言的UIという革命**

Reactを学ぶ上で、最初に理解すべき最も重要なコンセプトが「**宣言的UI (Declarative UI)**」です。この考え方こそ、Reactが現代のフロントエンド開発に革命をもたらした理由そのものです。

#### **1.1.1 UI構築の歴史：命令形から宣言形へ**

かつて、jQueryが一世を風靡した時代、私たちはUIを「**命令的 (Imperative)**」に作っていました。これは、「**どのように (How)** UIを変化させるか」を、一つひとつ手順を追ってコードで指示するやり方です。

**たとえ話：カーナビ**
*   **命令的な指示:** 「ここを直進200m、次の信号を右折、3つ目の角を左折…」
*   **宣言的な指示:** 「最終目的地は東京タワーです」

命令的な指示は、手順が複雑になるほど間違えやすく、現在地が変わると全ての指示が役に立たなくなる可能性があります。一方、宣言的な指示は、目的地さえ伝えれば、途中のルートはカーナビ（React）が現在地に応じて最適に計算してくれます。

**コードで見る命令的UI (jQuery風)**

例えば、「ボタンを押したらメッセージを表示し、ボタンを無効化する」というUIを考えてみましょう。
...
このコードの課題は、UIの状態（`isClicked`のような変数）を直接管理せず、DOM（画面の要素）を直接操作して「手順」を記述している点です。UIが複雑になり、「もしユーザーがログインしていなかったら」「もしエラーが発生したら」といった条件が加わると、この「手順書」はあっという間にスパゲッティコードになってしまいます。

#### **1.1.2 Reactの登場：宣言的UIの夜明け**

Reactは、このアプローチを根本から変えました。Reactでは、「**UIがあるべき姿 (What)**」を**状態 (State)** の関数として宣言します。
...
状態が変わると、Reactが自動的に前回のUIとの差分を計算し、DOMへの最小限の変更を効率的に行ってくれます。私たちは、UIをどう変化させるかという面倒な手順を考える必要がなくなり、アプリケーションの「状態」の管理に集中できるのです。これこそが、Reactがもたらした革命的な変化です。

##### **【図解】命令的UIと宣言的UIの処理フローの違い**

この違いをシーケンス図で見てみましょう。ユーザーがボタンをクリックした後の、開発者とライブラリの役割の違いがよくわかります。
...
sequenceDiagram
    ...
    User->>jQuery: ボタンをクリック
    activate jQuery
    jQuery->>Developer: イベント発生！(click)
    activate Developer

    Developer->>DOM: (命令) p#messageを探せ
    Developer->>DOM: (命令) テキストを'...'に変更しろ
    Developer->>DOM: (命令) 色を'red'に変更しろ
    Developer->>DOM: (命令) button#myButtonを探せ
    Developer->>DOM: (命令) テキストを'...'に変更しろ
    Developer->>DOM: (命令) disabledをtrueにしろ
    deactivate Developer
    deactivate jQuery
    Note over Developer, DOM: 開発者がDOM操作の「手順」を全て指示

    %% ---- Reactの場合 ----

    User->>React: ボタンをクリック
    activate React
    React->>Developer: イベント発生！(onClick)
    activate Developer

    Developer->>React: (依頼) stateを isClicked: true に更新して
    deactivate Developer

    React->>React: 状態(state)が変化したことを検知
    React->>React: 新しいstateでUIを再計算 (宣言)
    React->>DOM: 計算した差分だけをDOMに適用
    deactivate React
    Note over React, DOM: 開発者は「状態」を変えるだけ。<br/>DOM操作はReactが担当。
```

#### **1.1.3 宣言的UIの根幹：仮想DOMと再調整 (Reconciliation)**

Reactが宣言的UIを実現する上で最も重要な技術が「**仮想DOM (Virtual DOM)**」と「**再調整 (Reconciliation)**」の仕組みです。

**仮想DOMとは？**
仮想DOMは、実際のブラウザDOMを模した、軽量なJavaScriptオブジェクトです。Reactは、コンポーネントのStateやPropsが変わるたびに、新しい仮想DOMツリーをメモリ上に作ります。

**再調整 (Reconciliation) のプロセス**
1.  **状態の変更:** `useState`のセッター関数などが呼ばれ、コンポーネントの状態が更新されると、Reactは新しい仮想DOMツリーを生成します。
2.  **差分検出 (Diffing):** Reactは、新しく生成された仮想DOMツリーと、前回の仮想DOMツリーを比較し、両者の「差分」を効率的に見つけ出します。この差分検出アルゴリズムは、いくつかの経験則に基づいて最適化されており、非常に高速です。
    *   **要素のタイプが違う場合:** 例えば、`<div>`が`<span>`に変わったら、Reactは古いDOMノードを破棄し、新しいノードを完全に作り直します。
    *   **要素のタイプが同じ場合:** 例えば、`<div>`が`<div>`のままなら、Reactは属性（Props）の変更だけをチェックし、変わった属性だけを実際のDOMに適用します。
    *   **子要素の再帰処理:** 子要素に対しても同じ差分検出を再帰的に行います。リスト要素の場合、`key`属性が非常に重要になります（後述）。
3.  **実際のDOMへの適用:** 検出された最小限の差分だけを、実際のブラウザDOMに効率的に適用します。このプロセスは「**コミット (Commit)**」と呼ばれます。
...
```mermaid
graph TD
    A["状態(State)が変更される"] --> B{"Reactが新しい仮想DOMを生成"};
    B --> C{"前回の仮想DOMと比較 (Diffing)"};
    C --> D[差分だけを抽出];
    D --> E{"実際のDOMに変更を適用 (Commit)"};
    E --> F[ブラウザの画面が更新される];

    subgraph Render Phase a["(計算フェーズ)"]
        B;
        C;
        D;
    end

    subgraph Commit Phase b["(適用フェーズ)"]
        E;
    end

    style Render Phase fill:#e6f3ff,stroke:#333,stroke-width:2px
    style Commit Phase fill:#e6ffed,stroke:#333,stroke-width:2px
```

**なぜこの仕組みが必要なのか？**
実際のブラウザDOMへの操作は、非常にコストの高い（時間がかかる）処理です。頻繁なDOM操作はパフォーマンスの低下を招き、UIの描画を遅くします。Reactは、仮想DOM上で変更をまとめて計算し、実際のDOM操作を最小限に抑えることで、高いパフォーマンスとスムーズなUI更新を実現しているのです。開発者はDOM操作の詳細を意識することなく、アプリケーションの状態に集中できます。
...
優れたコンポーネントは、**単一責任の原則 (Single Responsibility Principle)** に従います。つまり、**一つのコンポーネントは、一つのことだけに責任を持つべき**です。これにより、コンポーネントは理解しやすく、テストしやすく、再利用しやすくなります。
...
状態`money`を親である`MarketPage`に「リフトアップ（持ち上げ）」し、子コンポーネントにPropsとして渡す様子を図で見てみましょう。データ（状態）は親から子へ、イベント（更新依頼）は子から親へと流れます。

```mermaid
graph TD
    subgraph MarketPage (親)
        A[State: money]
        B(Handler: handleBuy)
        A -- "props.amount (値)" --> C[MoneyDisplay]
        B -- "props.onBuy (関数)" --> D{ShopItem}
    end

    subgraph MoneyDisplay (子)
        C --> E[表示: {money}]
    end
...
##### **【図解】制御されたコンポーネントのデータフロー**

ユーザーの入力がどのようにStateを経由してUIに反映されるかを見てみましょう。データが一方向に流れるループを形成します。

```mermaid
sequenceDiagram
    actor User
    participant InputDOM as <input>要素
    participant React
    participant State

    User->>InputDOM: 'A'と入力
    InputDOM->>React: onChangeイベント発火
    activate React
    React->>State: setName('A') を実行
    React->>React: Stateの変更を検知し、再レンダリング
    React->>InputDOM: value="A" を設定
    deactivate React
    Note right of React: Stateが常に真実の情報源(Source of Truth)となる
```
